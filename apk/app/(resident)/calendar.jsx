...
export default function ResidentCalendar() {
  ...
  const fetchBookings = async (silent = false) => {
    try {
      ...
      const res = await getMyBookings({ status: 'all' });
  ...
  return (
    <FadeInView style={styles.container}>
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchBookings(true); }} />}
      >
        <View style={styles.headerCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.subtitle}>Track upcoming inspections and work</Text>
          </View>
        </View>
            ...
            {sortedSelectedBookings.map((b) => {
              ...
              return (
                <View key={`${b._id}-${b.eventType}`} style={styles.card}>
                  ...
                  <Text style={styles.viewBtn} onPress={() => router.push(`/(resident)/booking/${b._id}`)}>View Details</Text>
                </View>
              );
            })}
          </View>
