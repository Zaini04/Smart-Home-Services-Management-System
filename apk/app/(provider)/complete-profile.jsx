...
import { completeProfile, getCategoriesWithSkills, getProviderStatus } from '../../src/api/serviceProviderEndPoints';
...
export default function CompleteProfileScreen() {
  ...
  const fetchData = async () => {
    try {
      if (user?._id) {
        const statusRes = await getProviderStatus(user._id);
        setProviderStatus(statusRes?.data?.data?.status || null);
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        {['pending', 'waiting'].includes(providerStatus) ? (
          <View style={styles.pendingBox}>
            ...
            <Text style={styles.pendingText}>
              ...
              Until approval, only this page is accessible.
            </Text>
          </View>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  pendingTitle: { marginTop: 8, fontSize: 18, fontWeight: '700', color: '#92400E' },
  pendingText: { marginTop: 6, fontSize: 13, color: '#B45309', textAlign: 'center', lineHeight: 19 },
