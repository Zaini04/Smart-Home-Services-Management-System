...
const getOfferFromJob = (job) => job?.myOffer || null;
...
export default function AvailableJobsScreen() {
  ...
  const handleSendOfferPress = (job) => {
    ...
    const myOffer = getOfferFromJob(job);
    setSelectedJob(job);
    setLaborCost(myOffer?.laborEstimate ? String(myOffer.laborEstimate) : '');
  ...
  const handleSubmitOffer = async () => {
    ...
    try {
      ...
      const isUpdating = !!getOfferFromJob(selectedJob);
      Alert.alert("Success", isUpdating ? "Your offer has been updated successfully!" : "Your offer has been sent successfully!");
      setSelectedJob(null);
  ...
  const renderJobCard = ({ item }) => (
    <View style={styles.jobCard}>
      ...
      <View style={styles.actionRow}>
        ...
        <TouchableOpacity 
          style={[styles.offerBtn, item.hasBid && styles.offerBtnUpdate]} 
          onPress={() => handleSendOfferPress(item)}
          activeOpacity={0.8}
        >
          <Text style={[styles.offerBtnText, item.hasBid && styles.offerBtnTextUpdate]}>
            {item.hasBid ? 'Update Offer' : 'Send Offer'}
          </Text>
          <Ionicons name="arrow-forward" size={14} color={item.hasBid ? '#111827' : '#FFF'} />
        </TouchableOpacity>
      </View>
    </View>
  );
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>{selectedJob?.hasBid ? 'Update Offer' : 'Send Offer'}</Text>
              ...
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                  ...
                  <View style={styles.jobPreviewCard}>
                    ...
                    {selectedJob?.myOffer && (
                      <Text style={{ marginTop: 8, fontSize: 12, color: Colors.textSecondary }}>
                        Existing offer: Rs. {Number(selectedJob.myOffer.laborEstimate || 0).toLocaleString()}
                      </Text>
                    )}
                  </View>
                )}
                ...
                >
                  {isSubmitting ? (
                    ...
                    <Text style={styles.submitBtnText}>{selectedJob?.hasBid ? 'Update Offer' : 'Submit Offer'}</Text>
                  )}
                </TouchableOpacity>
              </ScrollView>
            </View>
...
const styles = StyleSheet.create({
  ...
  offerBtnUpdate: { backgroundColor: '#FACC15' },
  offerBtnText: { fontSize: 13, fontWeight: '600', color: '#FFF' },
  offerBtnTextUpdate: { color: '#111827' },
