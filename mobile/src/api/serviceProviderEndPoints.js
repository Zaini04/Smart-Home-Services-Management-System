...
export const requestInspection = (bookingId, data) => {
  return axiosInstance.post(`/api/serviceProvider/request-inspection/${bookingId}`, data);
};

export const respondToCounterFee = (bookingId, data) => {
  return axiosInstance.post(`/api/serviceProvider/respond-counter-fee/${bookingId}`, data);
};

export const updatePriceDuringWork = (bookingId, data) => {
  return axiosInstance.post(`/api/serviceProvider/update-price/${bookingId}`, data);
};

export const updateSchedule = (bookingId, data) => {
  return axiosInstance.post(`/api/serviceProvider/update-schedule/${bookingId}`, data);
};

export const providerCancelJob = (bookingId, data) => {
  return axiosInstance.post(`/api/serviceProvider/cancel-job/${bookingId}`, data);
};
