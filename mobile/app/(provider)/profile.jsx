    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 20, paddingTop: 56 }}>
        ...
        <View style={styles.menuCard}>
          {[
            { icon: 'person-outline', label: 'Edit Profile', onPress: () => router.push('/(provider)/edit-profile') },
            { icon: 'wallet-outline', label: 'My Wallet', onPress: () => router.push('/(provider)/wallet') },
            { icon: 'cash-outline', label: 'Earnings', onPress: () => router.push('/(provider)/earnings') },
            { icon: 'document-text-outline', label: 'My Offers', onPress: () => router.push('/(provider)/my-offers') },
          ].map((item, i) => (
        ...
        {user?.isVerified && (
          <View style={styles.verifiedCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.verifiedText}>Your KYC documents are approved.</Text>
            </View>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  verifiedCard: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: '#F0FDF4', padding: 16, borderRadius: 16, marginBottom: 16, borderWidth: 1, borderColor: '#BBF7D0' },
  verifiedTitle: { fontSize: 15, fontWeight: '700', color: '#166534' },
  verifiedText: { fontSize: 13, color: '#15803D' },
