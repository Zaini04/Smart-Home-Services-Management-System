...
import LocationMarker from "../../components/resident/postJob/LocationMarker";
...
export default function PostJob() {
  ...
  const handleImageChange = (e) => {
    ...
    if (validFiles.length !== files.length) { setError("Only image files are allowed"); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        try {
          ...
          if (data?.display_name) { setAddress(data.display_name); setError(""); }
        } catch (err) { console.error(err); }
        finally { setGettingLocation(false); }
      },
      () => { setError("Location permission denied. Click the map manually."); setGettingLocation(false); }
    );
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50">
        <div className="bg-white p-8 rounded-2xl shadow-xl text-center max-w-md mx-4">
          ...
          <p className="text-gray-600 mb-4">Workers will start sending their offers soon.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <FaList className="text-blue-600" /> Select Category <span className="text-red-500">*</span>
              </h3>
              {categoriesLoading ? (
                <div className="flex justify-center py-8"><FaSpinner className="w-8 h-8 text-blue-500 animate-spin" /></div>
              ) : categories.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-gray-500">No categories available</p>
                  <button type="button" onClick={fetchCategories} className="mt-2 text-blue-600 hover:underline">Try again</button>
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {categories.map((category) => (
                    ...
                    >
                      <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-xl font-bold mb-2 ${
                        selectedCategory?._id === category._id ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-500"
                      }`}>
                        {category.name?.charAt(0).toUpperCase()}
                      </div>
                      <p className={`font-medium text-sm ${selectedCategory?._id === category._id ? "text-blue-700" : "text-gray-700"}`}>
                        {category.name}
                      </p>
                      {selectedCategory?._id === category._id && (
                        <div className="absolute top-2 right-2"><FaCheckCircle className="w-5 h-5 text-blue-500" /></div>
                      )}
                    </button>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <FaEdit className="text-blue-600" /> Describe Your Problem <span className="text-red-500">*</span>
              </h3>
              <textarea
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="Example: My ceiling fan is making noise and not rotating properly..."
                rows={4}
                required
              ...
              />
              <p className="text-xs text-gray-400 mt-2">Be as detailed as possible to get accurate quotes</p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-800 flex items-center gap-2">
                  <FaMapMarkerAlt className="text-blue-600" /> Your Address <span className="text-red-500">*</span>
                </h3>
                <button
                  type="button"
              ...
              {!showMap ? (
                ...
                <div className="h-64 w-full rounded-xl overflow-hidden border-2 border-gray-200 mb-3 relative z-0">
                  <MapContainer center={position || [30.3753, 69.3451]} zoom={position ? 15 : 5} style={{ height: "100%", width: "100%" }}>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
                <FaCamera className="text-blue-600" /> Add Photos
                <span className="text-gray-400 font-normal text-sm ml-1">(Optional)</span>
              </h3>
              <div className="flex flex-wrap gap-3">
                {images.map((img, index) => (
                  <div key={index} className="relative w-24 h-24 rounded-xl overflow-hidden group border-2 border-gray-200">
              ...
              <p className="text-xs text-gray-400 mt-3">Upload up to 5 photos • JPG, PNG supported</p>
            </div>
            ...
            >
              {loading ? <><FaSpinner className="animate-spin" /> Posting...</> : <><FaPaperPlane /> Post Job</>}
            </button>
          </form>
          <div className="h-8" />
        </div>
