...
const statusMeta = {
  ...
  inspection_requested: {
    ...
    message: "Inspection request sent. Waiting for resident to approve.",
  },
  inspection_approved: {
    step: 2,
        ...
        {steps.map((s) => {
          ...
          return (
            <div key={s.step} className="flex flex-col items-center z-10">
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${done ? "bg-blue-500 border-blue-500" : active ? "bg-white border-blue-500 shadow-md shadow-blue-200" : "bg-white border-gray-300"}`}
              >
                {done ? (
                  <FaCheckCircle className="w-4 h-4 text-white" />
                ) : (
                  <span
                    className={`text-xs font-bold ${active ? "text-blue-600" : "text-gray-400"}`}
                  >
                    {s.step}
                  </span>
                )}
              ...
              <span
                className={`text-xs mt-1 font-medium ${active ? "text-blue-600" : done ? "text-gray-600" : "text-gray-400"}`}
              >
                {s.label}
              </span>
            </div>
...
function CommissionPreview({ laborCost, walletBalance = 0 }) {
  ...
  return (
    <div
      className={`rounded-xl p-4 border mb-4 space-y-1.5 ${hasEnough ? "bg-white border-green-200" : "bg-red-50 border-red-200"}`}
    >
      ...
      <div className="flex justify-between text-sm">
        ...
        <span className="text-red-600">
          - Rs. {comm.finalCommission.toLocaleString()}
        </span>
      </div>
      ...
      {!hasEnough && (
        <div className="bg-red-100 rounded-lg p-2 mt-2 flex items-start gap-2">
          ...
          <p className="text-red-700 text-xs">
            Wallet needs Rs. {comm.finalCommission.toLocaleString()} for
            commission. Balance: Rs. {walletBalance.toLocaleString()}
          </p>
        </div>
      )}
    </div>
...
function InspectionAndPricePanel({
  isInspection,
  onSubmit,
  loading,
  walletBalance,
  ...
  const handleSubmit = () => {
    ...
    if (comm && walletBalance < comm.finalCommission) {
      setError(`Need Rs. ${comm.finalCommission.toLocaleString()} in wallet`);
      return;
    }
    setError("");
    onSubmit({
  ...
  return (
    <div
      className={`bg-${accentColor}-50 border-2 border-${accentColor}-200 rounded-2xl p-5`}
    >
      <div className="flex items-start gap-3 mb-4">
        <div
          className={`w-10 h-10 bg-${accentColor}-100 rounded-xl flex items-center justify-center flex-shrink-0`}
        >
          {isInspection ? (
      ...
      <div className="space-y-3 mb-4">
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-200">
          <h4 className="font-medium text-gray-800 text-sm mb-3 flex items-center gap-2">
            <FaCalendarAlt className="text-blue-600" /> Schedule *
          </h4>
              ...
              <div className="w-32">
                ...
                <div className="flex border-2 border-gray-200 rounded-lg overflow-hidden">
                  <button
                    ...
                    className={`flex-1 py-2.5 text-sm font-medium transition-all ${durationUnit === "hours" ? "bg-blue-600 text-white" : "bg-white text-gray-600"}`}
                  >
                    Hours
                  </button>
                  <button
                    ...
                    className={`flex-1 py-2.5 text-sm font-medium transition-all ${durationUnit === "days" ? "bg-blue-600 text-white" : "bg-white text-gray-600"}`}
                  >
                    Days
                  </button>
                </div>
    ...
    <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-5">
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-amber-700 text-sm mt-0.5">
            Set a time to visit and inspect before giving final price.
          </p>
        </div>
      </div>
      <div className="space-y-3 mb-4">
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full px-3 py-2.5 rounded-xl border-2 border-amber-200 outline-none text-sm"
            />
          </div>
          <div className="flex-1">
            <label className="block text-sm font-medium text-gray-700 mb-1">
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Why do you need to inspect?"
            rows={2}
      ...
      <div className="flex gap-3">
        <button
          ...
          className="flex-1 py-2.5 border-2 border-gray-200 rounded-xl text-gray-600 font-medium bg-white"
        >
          Cancel
        </button>
        <button
        ...
        >
          {loading ? <FaSpinner className="animate-spin" /> : <FaSearch />} Send
          Request
        </button>
      </div>
    </div>
...
function PriceRevisionPanel({ booking, onSubmit, loading, walletBalance }) {
  const [expanded, setExpanded] = useState(false);
  const [laborCost, setLaborCost] = useState(
    booking.finalPrice?.laborCost?.toString() || "",
  );
  ...
  if (!expanded)
    return (
      <button
        onClick={() => setExpanded(true)}
        className="w-full py-3 bg-orange-50 border-2 border-orange-200 text-orange-700 rounded-2xl font-medium flex items-center justify-center gap-2 hover:bg-orange-100"
      >
  ...
  return (
    <div className="bg-orange-50 border-2 border-orange-200 rounded-2xl p-5">
      ...
      <div className="space-y-3 mb-4">
        ...
        <div>
          ...
          <textarea
            ...
            rows={2}
            className="w-full px-4 py-2.5 rounded-xl border-2 border-gray-200 outline-none text-sm resize-none"
          />
        </div>
      </div>
      <CommissionPreview laborCost={laborCost} walletBalance={walletBalance} />
      <div className="flex gap-3">
        <button
          onClick={() => setExpanded(false)}
          className="flex-1 py-2.5 border-2 border-gray-200 rounded-xl bg-white"
        >
          Cancel
        </button>
        <button
          onClick={handleSubmit}
          disabled={loading}
          className="flex-1 py-2.5 bg-gradient-to-r from-orange-500 to-amber-500 text-white rounded-xl font-semibold disabled:opacity-60"
        >
...
function UpdateSchedulePanel({ booking, onSubmit, loading }) {
  ...
  if (!expanded)
    return (
      <button
        onClick={() => setExpanded(true)}
        className="w-full py-3 bg-purple-50 border-2 border-purple-200 text-purple-700 rounded-2xl font-medium flex items-center justify-center gap-2 hover:bg-purple-100"
      >
    ...
    <div className="bg-purple-50 border-2 border-purple-200 rounded-2xl p-5">
      ...
      <div className="space-y-3 mb-4">
        <div>
          ...
          <input
            ...
            min={new Date().toISOString().slice(0, 16)}
            className="w-full px-3 py-2.5 rounded-lg border-2 border-gray-200 outline-none text-sm"
          />
        </div>
        <div className="flex gap-2">
          <div className="flex-1">
          ...
          <div className="w-32">
            ...
            <div className="flex border-2 border-gray-200 rounded-lg overflow-hidden">
              <button
                type="button"
                onClick={() => setDurationUnit("hours")}
                className={`flex-1 py-2.5 text-sm font-medium ${durationUnit === "hours" ? "bg-purple-600 text-white" : "bg-white text-gray-600"}`}
              >
                Hours
              </button>
              <button
                type="button"
                onClick={() => setDurationUnit("days")}
                className={`flex-1 py-2.5 text-sm font-medium ${durationUnit === "days" ? "bg-purple-600 text-white" : "bg-white text-gray-600"}`}
              >
                Days
              </button>
            </div>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          disabled={loading}
          className="flex-1 py-2.5 bg-purple-600 text-white rounded-xl font-semibold disabled:opacity-60"
        >
...
function StartWorkPanel({ onStart, loading }) {
  return (
    <div className="bg-teal-50 border-2 border-teal-200 rounded-2xl p-5 text-center">
      ...
      >
        {loading ? <FaSpinner className="animate-spin" /> : <FaTools />} Start
        Work Now
      </button>
    </div>
  );
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-indigo-700 text-sm mt-0.5">
            Show the Complete OTP to the resident.
          </p>
        </div>
      </div>
      {code && (
    ...
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
      ...
      <div className="space-y-2">
        {revisions.map((rev, i) => (
          <div
            key={rev._id || i}
            className="text-sm p-2.5 rounded-lg bg-gray-50"
          >
            <div className="flex justify-between items-center">
              <span className="text-gray-600">
                Rs. {rev.totalAmount?.toLocaleString()}
              </span>
              <span
                className={`px-2 py-0.5 rounded-full text-xs font-medium ${rev.status === "approved" ? "bg-green-100 text-green-700" : rev.status === "rejected" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}
              >
                {rev.status}
              </span>
            </div>
            {rev.reason && (
              <p className="text-gray-400 text-xs mt-1">{rev.reason}</p>
            )}
          </div>
        ))}
      </div>
...
function ConflictModal({ 
  ...
  const endTime = new Date(conflict.end).toLocaleString("en-US", {
    hour: "2-digit",
    minute: "2-digit",
  });
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
        ...
        <h3 className="text-xl font-bold text-gray-800 text-center mb-2">
          Cannot Reach Next Job
        </h3>
        
        <p className="text-gray-500 text-sm text-center mb-5">
          {conflict.message || "You have another job scheduled"}
        </p>
        ...
        <div className="bg-orange-50 border-2 border-orange-200 rounded-xl p-4 mb-5">
          ...
          <div className="bg-white rounded-lg p-3 border border-orange-200">
            <div className="flex items-center gap-2 text-sm mb-1">
              <FaCalendarAlt className="text-orange-600 w-4 h-4" />
        ...
        {/* Action Buttons */}
        <div className="space-y-3">
          {/* Button 1: Reduce extension */}
          <button
            onClick={onPickAnother}
            className="w-full py-3.5 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all flex items-center justify-center gap-2"
          >
            <FaEdit className="w-4 h-4" />
            Reduce Extension
          </button>

          {/* Button 2: Update conflicting job */}
          <button
            onClick={() => onMessage(conflict._id)}
          ...
          >
            📅 Update {conflict.residentName}'s Schedule
          </button>
...
/* ─────────────────────────────────────────
   UPDATE PENDING SCHEDULE PANEL
   For price_approved jobs (not started yet)
───────────────────────────────────────── */
function UpdatePendingSchedulePanel({ booking, onSubmit, loading }) {
  ...
  if (!expanded)
    return (
      <button
        onClick={() => setExpanded(true)}
        className="w-full py-3 bg-blue-50 border-2 border-blue-200 text-blue-700 rounded-2xl font-medium flex items-center justify-center gap-2 hover:bg-blue-100"
      >
    ...
    <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-5">
      ...
      {currentSchedule && (
        <div className="bg-white rounded-lg p-3 mb-3 border border-blue-200">
          ...
          <p className="text-sm font-medium text-gray-800">
            {currentSchedule.dateStr} at {currentSchedule.timeStr}
          </p>
          <p className="text-xs text-gray-600">
            Duration: {currentSchedule.durStr}
          </p>
        </div>
      )}
      ...
      <div className="space-y-3 mb-4">
          ...
          <div className="w-32">
            ...
            <div className="flex border-2 border-gray-200 rounded-lg overflow-hidden">
              <button
                type="button"
                onClick={() => setDurationUnit("hours")}
                className={`flex-1 py-2.5 text-sm font-medium ${durationUnit === "hours" ? "bg-blue-600 text-white" : "bg-white text-gray-600"}`}
              >
                Hours
              </button>
              <button
                type="button"
                onClick={() => setDurationUnit("days")}
                className={`flex-1 py-2.5 text-sm font-medium ${durationUnit === "days" ? "bg-blue-600 text-white" : "bg-white text-gray-600"}`}
              >
                Days
              </button>
            </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
          <p className="text-xs text-yellow-800">
            ⚠️ Resident must approve the new schedule
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          disabled={loading || !startDate || !durationValue}
          className="flex-1 py-2.5 bg-blue-600 text-white rounded-xl font-semibold disabled:opacity-60"
        >
...
const handleCompleteInspection = async (data) => {
  try {
    setActionLoading(true);
    await completeInspection(bookingId, data);

    showAlert("success", "Price sent to resident!");
    refreshLocal();
    
  } catch (err) {
    const apiError = err.response?.data;
    showAlert("error", apiError?.message || "Failed");
  } finally {
    setActionLoading(false);
  }
};
const handleSendPrice = async (data) => {
  try {
    setActionLoading(true);
    await sendFinalPrice(bookingId, data);
    ...
    const apiError = err.response?.data;
    // ✅ Just show the detailed error message
    showAlert("error", apiError?.message || "Failed to send price");
  } finally {
    setActionLoading(false);
  }
};

const handleUpdatePendingSchedule = async (data) => {
  try {
    setActionLoading(true);
    await updatePendingSchedule(bookingId, data);

    showAlert("success", "Schedule updated! Resident must re-approve.");
    refreshLocal();

  } catch (err) {
    const apiError = err.response?.data;
    showAlert("error", apiError?.message || "Failed to update schedule");
  } finally {
    setActionLoading(false);
  }
};
  const handleStartWork = async () => {
    try {
      ...
      showAlert("success", "Work started!");
      refreshLocal();
    } catch (err) {
  ...
  const handleCompleteWork = async () => {
    try {
      ...
      showAlert(
        "success",
        "Work marked complete! Ask resident to confirm with OTP.",
      );
      refreshLocal();
    } catch (err) {
...
const handleUpdateSchedule = async (data) => {
  ...
  const handleCancelJob = async () => {
    try {
      setActionLoading(true);
      await providerCancelJob(bookingId, { reason: cancelReason });
      setShowCancelModal(false);
      showAlert("success", "Job cancelled.");
      refreshLocal();
    } catch (err) {
  ...
  const handleRespondToCounter = async (action, newFee) => {
    try {
      ...
      showAlert(
        "success",
        action === "accept" ? "Counter offer accepted!" : "New fee proposed!",
      );
      refreshLocal();
    } catch (err) {
      showAlert("error", err.response?.data?.message || "Failed");
...
//   try {
//     await requestReschedule(conflictBookingId, {
//       proposedStartDate: newStartDate,
//     });

//     showAlert("success", "Request sent to resident");
//     setShowRescheduleUI(false);
//   } catch (err) {
...
// };
    ...
    <>
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 py-8 px-4">
        <div className="max-w-4xl mx-auto">
          {alertMsg && (
            <div
              className={`mb-4 p-4 rounded-xl flex items-start gap-3 ${alertMsg.type === "success" ? "bg-green-50 border border-green-200 text-green-800" : "bg-red-50 border border-red-200 text-red-800"}`}
            >
  ...
  <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-5">
    ...
    <p className="text-sm text-red-700 mb-2">
      You already have a booking with{" "}
      <strong>{conflictData.residentName}</strong>
    </p>

    <p className="text-sm text-gray-700 mb-1">
      Service: <strong>{conflictData.category}</strong>
    </p>
    ...
    <button
      onClick={() => {
        setShowRescheduleUI(true);
        setConflictBookingId(conflictData.bookingId);
      }}
      ...
      Send Request
                ...
                <div className="bg-yellow-50 border-2 border-yellow-200 rounded-2xl p-5 text-center">
                  ...
                  {booking.inspection?.status === "counter_offered" && (
                    <div className="mt-3 flex gap-2 justify-center">
                      ...
                      >
                        Accept Rs. {booking.inspection.counterFee}
                      </button>
                      <button
                        onClick={() => {
              ...
              {booking.status === "price_approved" && !otpVerified && (
                ...
                <VerifyOTPPanel
                  ...
                  loading={actionLoading}
                />
                 <UpdatePendingSchedulePanel
      booking={booking}
      onSubmit={handleUpdatePendingSchedule}
      loading={actionLoading}
    />
    </>
              )}
            ...
            <div className="space-y-4">
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800 text-sm">
                      {booking.resident?.full_name || "Resident"}
                    </p>
                    ...
                    <>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-red-600 font-medium">
                          - Rs.{" "}
                          {(booking.commission?.amount || 0).toLocaleString()}
                        </span>
                      </div>
                      <div className="border-t pt-2 flex justify-between font-bold text-green-700">
                        <span>Your Earning</span>
                    ...
                    <div className="border-t pt-2 mt-2 space-y-1.5">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-gray-700 text-xs">
                          {schedule.dateStr} {schedule.timeStr}
                        </span>
                      </div>
                      {schedule.durStr && (
                        <div className="flex justify-between">
              ...
              {canCancel && (
                <button
                  ...
                  className="w-full py-3 border-2 border-red-200 text-red-600 rounded-2xl font-medium hover:bg-red-50 transition-all text-sm flex items-center justify-center gap-2"
                >
      ...
      {showCancelModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
            ...
            {/* Conflict Modal - only for extending during work */}
      <ConflictModal
        conflict={conflictModal}
        isExtending={conflictModal?.isExtending || false}
        onClose={() => setConflictModal(null)}
        onMessage={(conflictBookingId) => {
          setConflictModal(null);
          navigate(`/provider/job/${conflictBookingId}`);
        }}
        onPickAnother={() => {
          setConflictModal(null);
          showAlert("info", "Reduce the extension below.");
        }}
      />
    </>
  );
