// pages/resident/bookingDetails/panels/BookingProgressBar.jsx
import React from "react";
import { FaCheckCircle, FaTimesCircle } from "react-icons/fa";
import { lifecycleSteps } from "./bookingConstants";

function BookingProgressBar({ currentStep, cancelled }) {
  if (cancelled) {
    return (
        ...
        {lifecycleSteps.map((s) => {
          ...
          return (
            <div key={s.step} className="flex flex-col items-center z-10">
              ...
              >
                {done ? (
                  <FaCheckCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white" />
                ) : (
                  <span className={`text-[10px] sm:text-xs font-bold ${active ? "text-blue-600" : "text-gray-400"}`}>
                    {s.step}
                  </span>
                )}
              </div>
              <span
                className={`text-[9px] sm:text-xs mt-1 font-medium text-center px-0.5 leading-tight ${
